...
def save_report():
    with open(os.path.join(OUT, "results_s1g.json"), "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2, default=str)
    with open(os.path.join(OUT, "spike_s1g.log"), "w", encoding="utf-8") as f:
        f.write("\n".join(log_lines))
...
def main():
    ...
    try:
        ...
        inserted = None
        # 最终变体：选中视图后 AllViews=False 定向导入
        annos = []
        matrix = []
        def try_ima(tag, fn):
            try:
                ret = fn()
                n = count_dims(view, mod)
                matrix.append({"tag": tag, "ret": str(ret)[:40], "dims_cum": n})
                log(f"{tag}: ret={ret} dims_cum={n}")
            except Exception as ex1:
                matrix.append({"tag": tag, "ex": str(ex1)[:100]})
                log(f"{tag} EX: {ex1}")
        ext0 = wrap(act, 'IModelDoc2', mod).Extension
        # 清选后选中视图
        try:
            mdoc0 = wrap(act, 'IModelDoc2', mod)
            mdoc0.ClearSelection2(True)
            sel_ok = ext0.SelectByID2(vname, "DRAWINGVIEW", 0, 0, 0, False, 0, None, 0)
            results["select_view"] = bool(sel_ok)
            log(f"select_view: {sel_ok}")
        except Exception as exs:
            results["select_view"] = f"EX {exs}"
            log(f"select_view EX: {exs}")
        try_ima("IMA2_marked_alltypes", lambda: drw2.InsertModelAnnotations2(32768, True, 0, True, True, True))
        try_ima("IMA2_notmarked_alltypes", lambda: drw2.InsertModelAnnotations2(524288, True, 0, True, True, True))
        try_ima("IMA2_notmarked_types", lambda: drw2.InsertModelAnnotations2(524288, False, 0x3FFFFFF, True, True, True))
        try_ima("IMD_opt0", lambda: drw2.InsertModelDimensions(0))
        try_ima("IMD_opt1", lambda: drw2.InsertModelDimensions(1))
        try_ima("IMD_opt2", lambda: drw2.InsertModelDimensions(2))
        ...
        out_png = os.path.join(OUT, "S1g-snapshot.png")
        t0 = time.time()
