# -*- coding: utf-8 -*-
"""Spike 001 S-1: 插视图(模型路径字符串) + InsertModelAnnotations3 + 尺寸枚举/删除 + S-2 快照"""
...
def save_report():
    with open(os.path.join(OUT, "results_s1.json"), "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2, default=str)
    with open(os.path.join(OUT, "spike_s1.log"), "w", encoding="utf-8") as f:
        f.write("\n".join(log_lines))
...
def main():
    ...
    try:
        ...
        inserted = None
        opts = 32767  # swInsertAll
        # 真实签名: IMA3(Option, Types, AllViews, DuplicateDims, HiddenFeatureDims, UsePlacementInSketch)
        annos = []
        for opt, tag in ((1, 'marked'), (2, 'not_marked')):
            key = f"insert_annotations_{tag}"
            try:
                inserted = drw2.InsertModelAnnotations3(opt, opts, True, False, False, False)
        ...
        if not targets and annos:
            targets = [wrap(a, 'IAnnotation', mod) for a in annos[:5]]
            results["delete_source"] = "IMA3_returned_IAnnotation"
        else:
        ...
        out_png = os.path.join(OUT, "S1-snapshot.png")
        t0 = time.time()
