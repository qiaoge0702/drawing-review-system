...
def save_report():
    with open(os.path.join(OUT, "results_s1c.json"), "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2, default=str)
    with open(os.path.join(OUT, "spike_s1c.log"), "w", encoding="utf-8") as f:
        f.write("\n".join(log_lines))
...
def main():
    ...
    try:
        ...
        inserted = None
        # 参数矩阵：Option × Types，找到能导入尺寸的组合
        annos = []
        matrix = []
        combos = [
            (32768, 8), (32768, 32768), (32768, 557064), (32768, 0x3FFFFFF),
            (524288, 8), (524288, 524288), (524288, 557064), (524288, 0x3FFFFFF),
            (557056, 557064), (557056, 0x3FFFFFF),
            (8, 8),
        ]
        for opt, types in combos:
            try:
                inserted = drw2.InsertModelAnnotations3(opt, types, True, True, True, False)
                n = len(inserted) if inserted else 0
                matrix.append({"opt": opt, "types": types, "n": n})
                log(f"IMA3 opt={opt} types={types}: n={n}")
                if n and not annos:
                    annos.extend(inserted)
                    results["winning_combo"] = {"opt": opt, "types": types, "n": n}
            except Exception as ex1:
                matrix.append({"opt": opt, "types": types, "ex": str(ex1)[:100]})
                log(f"IMA3 opt={opt} types={types} EX: {ex1}")
        ...
        out_png = os.path.join(OUT, "S1c-snapshot.png")
        t0 = time.time()
