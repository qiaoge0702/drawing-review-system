...
def save_report():
    with open(os.path.join(OUT, "results_s1f.json"), "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2, default=str)
    with open(os.path.join(OUT, "spike_s1f.log"), "w", encoding="utf-8") as f:
        f.write("\n".join(log_lines))
...
def main():
    ...
    try:
        ...
        def try_ima(tag, fn):
            try:
                inserted = fn()
                n = len(inserted) if inserted else 0
                matrix.append({"tag": tag, "n": n})
                log(f"{tag}: n={n}")
                if n and not annos:
                    annos.extend(inserted)
                    results["winning_combo"] = {"tag": tag, "n": n}
        ...
        out_png = os.path.join(OUT, "S1f-snapshot.png")
        t0 = time.time()
