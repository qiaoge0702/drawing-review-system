...
def save_report():
    with open(os.path.join(OUT, "results_s1d.json"), "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2, default=str)
    with open(os.path.join(OUT, "spike_s1d.log"), "w", encoding="utf-8") as f:
        f.write("\n".join(log_lines))
...
def main():
    ...
    try:
        ...
        inserted = None
        # 变体矩阵：先激活模型文档再回图纸；IMA3 / IMA2 / IMA1 三条路径
        annos = []
        matrix = []
        asm_title = P(asm.GetTitle)
        def try_ima(tag, fn):
            try:
                inserted = fn()
        ...
        sw.ActivateDoc3(drw_title, False, 0, 0)
        try_ima("IMA3_after_activate_opt8", lambda: drw2.InsertModelAnnotations3(8, 0x3FFFFFF, True, True, True, False))
        try_ima("IMA3_marked_after_activate", lambda: drw2.InsertModelAnnotations3(32768, 0x3FFFFFF, True, True, True, False))
        # b) IMA2(Option, AllViews, DuplicateDims, HiddenFeatureDims, UsePlacementInSketch) Option 1/2
        try_ima("IMA2_opt1", lambda: drw2.InsertModelAnnotations2(1, True, True, True, False))
        try_ima("IMA2_opt2", lambda: drw2.InsertModelAnnotations2(2, True, True, True, False))
        # c) IMA1(Option, AllViews, DuplicateDims)
        try_ima("IMA1_opt1", lambda: drw2.InsertModelAnnotations(1, True, True))
        try_ima("IMA1_opt2", lambda: drw2.InsertModelAnnotations(2, True, True))
        ...
        out_png = os.path.join(OUT, "S1d-snapshot.png")
        t0 = time.time()
